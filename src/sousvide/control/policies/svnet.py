...
class SVNet(nn.Module):    
    def __init__(self,
        ...
        if hasattr(vision_mlp_network, "set_vision_prompt"):
            vision_mlp_network.set_vision_prompt(config.get("vision_prompt", "null"))
        ...
        self.get_network.update({
            "Perception": {
                "Train":  self._perception_modules(self.network["VisionMLP"]),
                "Unlock": self._perception_modules(self.network["VisionMLP"]),
            }
        })

    def _perception_modules(self, vmlp: nn.Module) -> nn.ModuleList:
        mods = []
        vis = vmlp.networks[0]                         # VisionCNN wrapper
        enc = getattr(vis, "networks", None)           # actual backbone (e.g., CLIPSegSoftmaskEncoder)
        if enc is not None:
            # Always include the Dv→Nout projection so you can train it.
            if hasattr(enc, "proj"):
                mods.append(enc.proj)
            # If later you choose to fine-tune CLIPSeg's small decoder, add specific submodules here.
            # e.g., mods.append(enc.model)   # or pick enc.model.decoder subparts only
